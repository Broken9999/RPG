...
# Skill classes and dictionaries
class Skill:
...
# Item and Player classes with new stats and functionalities
class Item:
    def __init__(self, name, item_type, damage_bonus=0, health_bonus=0, mana_bonus=0, speed_bonus=0, durability=100):
        self.name = name
...
class Player:
    ...
    def get_initial_skills(self):
        return random.sample(skill_pool[self.player_class], 1)
...
# Monster and Area classes with new functionalities
class Monster:
...
# Game class with additional features
class Game:
    def __init__(self, player):
        ...
        self.special_items = {"Mana Rock": {"type": "special", "effect": "increase_random_stat", "price": 100}}
    ...
    def load_islands(self):
        islands = [
            ...
            Area("Dungeon",  "A hole in a cave with monsters all over it that rose to the top.",  20,  True,  True)
        ]
    ...
    def buy_potions(self):
        ...
        if potion_choice.title() in self.potions:
            potion_price = self.potions.get(potion_choice.title())
    ...
    def buy_special_items(self):
        print("\nSpecial Items for sale:")
        for item, details in self.special_items.items():
            print(f"{item}: {details['price']} coins (Effect: {details['effect']})")
        item_choice = input("Enter the name of the special item you want to buy: ").strip().lower()
        if item_choice.title() in self.special_items:
            item_price = self.special_items[item_choice.title()]["price"]
            if self.player.coins >= item_price:
                self.player.coins -= item_price
                item = Item(item_choice.title(), "special", durability=1)
                self.player.inventory["others"].append(item)
                print(f"You bought {item.name}.")
                self.apply_special_item_effect(item)
            else:
                print("Not enough coins.")
        else:
            print("Invalid special item choice.")

    def apply_special_item_effect(self, item):
        if item.name == "Mana Rock":
            stat_to_increase = random.choice(["health", "mana", "damage", "speed"])
            self.player.stats[stat_to_increase] += random.randint(10, 30)
            print(f"{stat_to_increase.capitalize()} increased by {self.player.stats[stat_to_increase]}!")
...
def main_game_loop():
    ...
    stats = {"level": 1, "xp": 0, "health": 100, "mana": 50, "damage": 10, "speed": 5, "element": random.choice(["Fire", "Ice", "Lightning", "Earth", "Water", "Air", "Darkness", "Light", "Poison", "Chaos"])}
